defmodule WCoreWeb.UserAuth do
  ...
  # ---------------------------------------------------------------------------
  # on_mount callbacks — usados por live_session no router para proteger
  # rotas LiveView. Funcionam da mesma forma que os plugs abaixo, mas operam
  # na session (cookie criptografado) em vez do conn.assigns.
  # ---------------------------------------------------------------------------

  @doc """
  Callback `on_mount` para LiveViews que exigem autenticação.

  Lê o token da session, busca o usuário no banco e atribui `current_scope`
  ao socket. Redireciona para login se não autenticado.
  """
  ...
  # Callback on_mount: redireciona usuários já autenticados (ex: página de login).
  ...
  # Lê o token da session e popula current_scope no socket.
  # Usa assign_new/3 para não sobrescrever se já foi atribuído por outro hook.
